...
export const resolvers = {
  Query: {
    ...
    issue: async (_: any, args: { id: string }): Promise<any> => {
      return Issue.findById( args.id ).populate( 'assignedTo' ).populate( 'comments' );
    },
  ...
  Mutation: {
    // - createUser
    createUser: async (_: any, args: { name: string; email: string }): Promise<any> => {
    ...
    // - updateIssueStatus
    updateIssueStatus: async (_: any, args: { id: string; status: "OPEN" | "ASSIGNED" | "IN_PROGRESS" | "RESOLVED" | "CLOSED" }): Promise<any> => {
    ...
    // - addComment
    addComment: async (_: any, args: { content: string; author: string; issueId: string }): Promise<any> => {
